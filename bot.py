"""
Crypto Trading Bot
- Datos de precio: Binance real (API pública, sin key)
- Ejecución de órdenes: Binance Testnet (con key)
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .stat-label{font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);margin-bottom:10px}
  ...
  .stat-sub{font-size:.7rem;color:var(--muted);margin-top:8px}
  ...
  th{padding:14px 20px;text-align:left;font-size:.65rem;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:400}
  td{padding:14px 20px;border-bottom:1px solid rgba(26,35,50,.5);vertical-align:middle}
  tr:last-child td{border-bottom:none}tr:hover td{background:rgba(255,255,255,.02)}
  .badge{display:inline-block;padding:3px 10px;border-radius:4px;font-size:.7rem;font-weight:700}
  ...
  .pair{color:#fff;font-weight:600}.ts{color:var(--muted);font-size:.72rem}
...
<div class="container">
  ...
  <div class="stats">
    <div class="stat-card" style="--accent:var(--blue)"><div class="stat-label">Total Trades</div><div class="stat-value" id="total">—</div><div class="stat-sub">operaciones ejecutadas</div></div>
    <div class="stat-card" style="--accent:var(--green)"><div class="stat-label">Compras</div><div class="stat-value green" id="buys">—</div><div class="stat-sub">órdenes de compra</div></div>
    <div class="stat-card" style="--accent:var(--red)"><div class="stat-label">Ventas</div><div class="stat-value red" id="sells">—</div><div class="stat-sub">órdenes de venta</div></div>
    <div class="stat-card" style="--accent:var(--yellow)"><div class="stat-label">Confianza promedio</div><div class="stat-value yellow" id="avg-conf">—</div><div class="stat-sub">score de Claude</div></div>
  </div>
  ...
  <div class="table-wrap">
    <table>
      <thead><tr><th>Par</th><th>Acción</th><th>Confianza</th><th>Señales</th><th>Razonamiento</th><th>Timestamp</th></tr></thead>
      <tbody id="trades-body"><tr><td colspan="6"><div class="empty"><div class="empty-icon">⏳</div><div class="empty-text">Cargando...</div></div></td></tr></tbody>
    </table>
  </div>
  <footer>CryptoBot Dashboard · Datos en tiempo real</footer>
</div>
<script>
let countdown=30;
async function loadData(){
  try{
    ...
    if(!trades.length){tbody.innerHTML='<tr><td colspan="6"><div class="empty"><div class="empty-icon">🤖</div><div class="empty-text">El bot está analizando señales...<br>Las operaciones aparecerán aquí cuando se ejecuten.</div></div></td></tr>';return;}
    tbody.innerHTML=[...trades].reverse().map(t=>{
      ...
      return `<tr><td class="pair">${t.symbol||'—'}</td><td><span class="badge badge-${(t.action||'').toLowerCase()}">${t.action||'—'}</span></td><td><div class="confidence-bar"><div class="bar-bg"><div class="bar-fill" style="width:${conf}%"></div></div><span style="font-size:.72rem;min-width:32px">${conf}%</span></div></td><td style="color:var(--muted)">${tSig} ${nSig}</td><td style="color:var(--muted);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${t.reasoning||''}">${t.reasoning||'—'}</td><td class="ts">${ts}</td></tr>`;
    }).join('');
...
def get_public_exchange():
    """Binance real — datos públicos sin key."""
    return ccxt.binance({"enableRateLimit": True, "options": {"defaultType": "spot"}})

def get_trade_exchange():
    """Binance Testnet — para ejecutar órdenes."""
    return ccxt.binance({
...
def ask_claude(symbol, t_sig, n_sig, df):
    ...
    prompt = f"""Sos un analista de trading crypto. Respondé SOLO en JSON sin backticks.
...
def run_bot():
    ...
    public_ex = get_public_exchange()   # para datos
    trade_ex  = get_trade_exchange()    # para órdenes
        ...
        for symbol in WATCHLIST:
            try:
                ...
                if abs(t_sig + n_sig) < MIN_SIGNALS:
                    log.info("  ⏭️  Señales insuficientes — skip"); continue
